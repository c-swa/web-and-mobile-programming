        ...
        speaker = new TextToSpeech(getApplicationContext(), new TextToSpeech.OnInitListener() {
            ...
            public void onInit(int status) {
                if(status < 0){
                    ...
                    speaker.speak("Welcome",TextToSpeech.QUEUE_FLUSH, null);
                }
            }
        });
    }
                ...
                if (result == RESULT_OK && intentData != null) {
                    ...
                    if (res.size() > 0) {
                        ...
                        if (res.get(0).equalsIgnoreCase("HELLO")) {
                            speaker.speak("What is your name?", TextToSpeech.QUEUE_FLUSH, null);
                        ...
                        // Retrieves user's name.
                            ...
                            speaker.speak(("Nice to meet you " + preferences.getString("NAME", "")), TextToSpeech.QUEUE_FLUSH, null);
                        ...
                        // Returns "I can understand. Please tell me your symptoms in short" if user says "I'm not feeling well"
                            ...
                            speaker.speak(I_CAN_UNDERSTAND, TextToSpeech.QUEUE_FLUSH, null);
                            mVoiceInputTv.append("\n" + I_CAN_UNDERSTAND);
                        }
                        // Says "thank you" if user also says "thank you"
                        else if (res.get(0).contains("thank")) {
                            speaker.speak(THANK_YOU + preferences.getString("NAME", "") + TAKE_CARE, TextToSpeech.QUEUE_FLUSH, null);
                            mVoiceInputTv.append("\n" + THANK_YOU + preferences.getString("NAME", "") + TAKE_CARE);
                        }
                        // Returns the time
                        else if (res.get(0).contains("time")) {
                            speaker.speak("The time is " + getCurrentTime(), TextToSpeech.QUEUE_FLUSH, null);
                            mVoiceInputTv.append("\n" + "The time is " + getCurrentTime());
                        }
                        // Suggests medicine
                        else if (res.get(0).contains("medicines")) {
                            speaker.speak(MEDICINE, TextToSpeech.QUEUE_FLUSH, null);
                            mVoiceInputTv.append("\n" + MEDICINE);
                        }
                        // If any issues or spoken words did not make sense to speech interpreter
                        else {
                            speaker.speak("I didn't understand you, can you say that again?", TextToSpeech.QUEUE_FLUSH, null);
